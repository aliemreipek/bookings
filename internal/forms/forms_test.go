...
func TestForm_Required(t *testing.T) {
	...
	form.Required("a", "b", "c")
	if !form.Valid() {
		t.Error("shows does not does not have required fields when it does")
	}
}
...
func TestForm_MinLength(t *testing.T) {
	...
	if !form.Valid() {
		t.Error("form does not show the field which minlength is greater than 1")
	}
...
func TestForm_IsEmail(t *testing.T) {
	...
	form.IsEmail("email")
	if form.Valid() {
		t.Error("got a valid for not a valid email address")
	}
